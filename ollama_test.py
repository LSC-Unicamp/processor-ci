...
def find_cpu_top_and_dependencies(repo_dir):
    verilog_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(repo_dir) for f in filenames if f.endswith('.v')]

    # Use Ollama to help identify the top-level CPU module and dependencies
    prompt = f"The repository contains the following Verilog files: {verilog_files}. Identify the top-level CPU module (they are usually identified by the names: top, core, cpu_top, cpu_csr or main) and list all the necessary Verilog files required for compilation."
    
    response = client.generate(model="qwen2.5:32b", prompt=prompt)
    print(response)

    if response and 'text' in response and ('cpu_top' in response['text'] or 'CPU top' in response['text']):
        try:
            # Extract top-level module and dependency files from Ollama's response
            cpu_top_start = response['text'].lower().index('cpu_top:')
            dependencies_start = response['text'].lower().index('dependencies:')

            # Get the CPU top file (assuming the file path follows 'cpu_top:')
            cpu_top_file = response['text'][cpu_top_start:].split(':')[1].strip().split()[0]

            # Get the list of dependencies (other necessary .v files)
            dependencies = response['text'][dependencies_start:].split(':')[1].strip().split()

            # Return the top-level CPU file and its dependencies
            return cpu_top_file, dependencies
        except ValueError as ve:
            print(f"Error parsing Ollama response: {ve}")
            sys.exit(1)
    else:
        print("Failed to identify the top-level CPU module or dependencies.")
        sys.exit(1)
...
def get_necessary_verilog_files_and_dirs(repo_dir):
    ...
    return necessary_files, necessary_dirs, cpu_top_file  # Return necessary files, directories, and top module
...
def write_json_output(repo_url, top_module, necessary_files, language_version, include_dirs, extra_flags):
    output_data = ' \
        "template": { \
            "repository":, \
            "name": "template", \
...
if __name__ == "__main__":
    ...
    write_json_output(repo_url, top_module, necessary_verilog_files, verilog_standard, necessary_verilog_dirs, extra_flags)
